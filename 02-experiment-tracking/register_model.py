...
# Disabling autologging due to version compatibility issue
...
def run_register_model(data_path: str, top_n: int):
    ...
    # Log the models and calculate test RMSE for each
    run_ids_and_rmses = []
    for run in runs:
        run_id, test_rmse = train_and_log_model(data_path=data_path, params=run.data.params)
        run_ids_and_rmses.append((run_id, test_rmse))

    # Select the model with the lowest test RMSE
    best_run_id, best_rmse = min(run_ids_and_rmses, key=lambda x: x[1])
